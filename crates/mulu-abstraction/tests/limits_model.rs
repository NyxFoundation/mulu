...
fn the_argument_partition_is_the_one_docs_11_works_through() {
    ...
    assert_eq!(
        regions.len(),
        3,
        "docs/11 §7 splits uint256 into three regions"
    );
    ...
    assert!(
        a.report
            .discharged
            .iter()
            .any(|d| d.contains("A and B and not spec:limit-bound")),
        "{:?}",
        a.report.discharged
    );
}
...
fn check_b_has_no_failing_transition_and_depends_on_a() {
    ...
    assert_eq!(
        b.depends_on,
        vec!["A".to_string()],
        "B is only reached once A passed"
    );
...
fn forceset_reaches_bad_and_setlimit_does_not() {
    ...
    assert_eq!(
        goes(&a, t2, "next_tx"),
        Some("idle_LIM0"),
        "revert restores the entry storage"
    );
}
...
fn every_event_is_uncontrollable_in_the_implementation_model() {
    ...
    assert!(a
        ...
        .events
        .iter()
        .all(|e| e.control == Control::Uncontrollable));
    let plant = a.model.control_plant.as_ref().expect("a reference plant");
    assert!(plant
        .events
        .iter()
        .any(|e| e.control == Control::Controllable));
    assert!(plant
        .events
        .iter()
...
fn without_a_specification_only_redundancy_is_modelled() {
    let (_, a) = build(false);
    assert!(
        a.model.bad.is_empty(),
        "no specification means no bad state"
    );
...
fn the_environment_profile_and_its_assumptions_are_recorded() {
    ...
    for expected in [
        "abi-decoder-unverified",
        "initial-state",
        "argument-regions-are-exact",
    ] {
        assert!(
            joined.contains(expected),
            "missing assumption {expected} in:\n{joined}"
        );
    }
    assert!(
        a.report.complete(),
